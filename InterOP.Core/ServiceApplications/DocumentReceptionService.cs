...
{
    public class DocumentReceptionService : IDocumentReceptionService
    {
        ...
        private const string vGetSearchAllProvider = "Search/Provider/all";


        public DocumentReceptionService(IConfiguration pvIPrvObConfig, IUnitOfWork pvUnitOfWork, IS3Services pvS3Services, IRedisCacheClient pvRedisCacheClient)
        {
        ...
        private async Task<List<EntProveedores>> LoadRedisProviders()
        {
            List<EntProveedores> vProvider;
            prvRedisBD = IPrvRedisCacheClient.GetDbFromConfiguration();
            vProvider = await prvRedisBD.GetAsync<List<EntProveedores>>(vGetSearchAllProvider);
            if (vProvider == null)
            {
                vProvider = (List<EntProveedores>)await prvUnitOfWork.ProviderRepository.GetProviders();
                await prvRedisBD.AddAsync(vGetSearchAllProvider, vProvider);
            }
            return vProvider;
        }
